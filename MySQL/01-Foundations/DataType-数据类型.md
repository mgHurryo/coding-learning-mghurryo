...
详见：[整数类型](../0-BASES/MySQL-Data-Types/整数类型.md)、[浮点类型](../0-BASES/MySQL-Data-Types/浮点类型.md)、[定点与位类型](../0-BASES/MySQL-Data-Types/定点与位类型.md)。
...
`fsp` 是小数秒精度，范围 0 到 6。跨系统事件通常统一存 UTC；纯日历语义不要强行转换成时间戳。详见：[日期类型](../0-BASES/MySQL-Data-Types/日期.md)。
...
`NCHAR` / `NVARCHAR` 是国家字符集兼容写法，不应代替显式的 `CHARACTER SET` 和 `COLLATE` 设计。详见：[字符串与二进制类型](../0-BASES/MySQL-Data-Types/字符串.md)、[枚举与集合类型](../0-BASES/MySQL-Data-Types/枚举与集合类型.md)、[字符集与排序规则](Charset-字符集与排序规则.md)。
...
JSON 不是“免设计”的替代品。频繁过滤或排序的路径应提升为普通列、generated column 或合适的函数/多值索引。详见：[JSON 类型](../0-BASES/MySQL-Data-Types/JSON-类型.md)。
...
空间字段应明确 SRID、坐标轴顺序、几何有效性和查询谓词。需要空间检索时使用 `SPATIAL INDEX` 并用 `EXPLAIN` 验证。详见：[空间数据类型](../0-BASES/MySQL-Data-Types/空间数据类型.md)。
...
主键、唯一、外键和 CHECK 是约束，不只是注释。CHECK 只拒绝 FALSE，UNKNOWN 会通过；需要必填输入时还应使用 `NOT NULL`。详见：[Column 字段](Column-字段.md)、[PRIMARY KEY](../02-Schema-DDL/PRIMARY-KEY-主键约束.md)、[UNIQUE](../02-Schema-DDL/UNIQUE-唯一约束.md)、[FOREIGN KEY](../02-Schema-DDL/FOREIGN-KEY-外键约束.md)、[NOT NULL](../02-Schema-DDL/NOT-NULL-非空约束.md)、[DEFAULT](../02-Schema-DDL/DEFAULT-默认值.md)。
